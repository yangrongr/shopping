	...
	<div>
		...
		<van-goods-action>
			...
			<div @click="onClickCollect">
				<van-goods-action-icon icon="star" text="已收藏" color="#ff5000" v-show="!showCollect"/>
				<van-goods-action-icon icon="star-o" text="收藏" v-show="showCollect"/>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			buy(){},
			onClickCollect(){
				var arr = this.collectGoods;
				if(this.showCollect == false){
					var that = this;
					arr.forEach(function(item,index){
						if(item == that.goodsId){
							arr.splice(index,1)
						}
					})
				...
				this.$cache_set('collect_goods_id', this.collectGoods)
			},
			initdata() {
				let that = this;
				goodsView({
			...
			selectGoodsInfo(){
				...
				if(select_data['selectedSkuComb'] == undefined || select_data['selectedSkuComb'] == ''){
					goods_data['price'] = select_data['goods']['sell_price']*100;
					goods_data['stock_num'] = select_data['goods']['store_nums'];
					goods_data['skuid'] = select_data['goods']['goods_no'];
				}else {
		...
		mounted() {
			...
			this.collectGoods = this.$cache_get('collect_goods_id')
			let arr = this.collectGoods;
			var that = this;
			arr.forEach(function(item,index){
				if(item == id){
