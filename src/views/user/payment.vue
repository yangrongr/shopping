	...
	export default {
		...
		methods: {
			...
			async pay(id) {
				...
				var remain = this.userInfo.balance
				if(parseInt(this.money) > parseInt(remain)){
					this.$toast('余额不足')
				}else{
					//付款
					...
					if(res.code == 200){
						if(res.msg == '订单支付成功')
						remain = parseInt(remain) - parseInt(this.money)
						this.userInfo['balance'] = remain + '';
						this.$cache_set('userInfo', this.userInfo)
						// console.log(this.userInfo)
